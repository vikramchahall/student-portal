...
export default function MealsCancelledPage() {
  ...
  useEffect(() => {
    fetchCancellations()
    // --- FIX 2: Update the interval to call the main fetch function ---
    // This ensures that every minute, it cleans up and then refreshes the data.
  ...
  const fetchCancellations = async () => {
    try {
      ...
      // First, cleanup past cancellations
  ...
  const cleanupPastCancellations = async () => {
    try {
      ...
      // --- FIX 1: REMOVED THE LINE BELOW TO BREAK THE INFINITE LOOP ---
